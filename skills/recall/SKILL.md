...
3. Scope when asked: `--task <branch-or-tag>` limits to one task's memories, `--author <email>` to one teammate's. Quarantined results (from low-trust imports) are marked `[quarantined]` — weigh them accordingly.
...
- Share memories with the team / import theirs: `/share` and `/share import`.

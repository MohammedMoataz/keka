...
2. Only if a hit looks relevant but truncated, expand: add `--full` — which also returns *every* match rather than the short preview's first few. Never start with `--full`: progressive disclosure is what keeps recall cheap.
